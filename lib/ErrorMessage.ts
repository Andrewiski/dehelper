...
export class ErrorMessage
{
                ...
                if(options.connectionInfo){
                    ...
                    if(options.request){
                        if(options.request instanceof Request){
                            this.connectionInfo = Helper.getExpressRequestConnectionInfo(options.request); ;
                        }
                    }
                }
                this.error = options;
    ...
    public static handleHttpRequestError = function (req : Request, res : Response, err : ErrorMessage, debugData : any, statusCode: number = 500) {
        let errorData = new ErrorMessage({error:err, debugData:debugData, request:req, statusCode:statusCode});
