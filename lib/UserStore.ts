...
export interface iUserStore {
  ...
  passwordHash_create(
    options: PasswordHashCreateOptions
  ): Promise<UserPassword>;
  usernamePassword_validate(
    options: UsernamePasswordValidateOptions
  ): Promise<User>;
...
export class UserStore implements iUserStore {
  constructor(userStoreCreateOptions: UserStoreCreateOptions | undefined) {
    ...
    userStoreOptions = {
      accessTokenExpiresIn:
        userStoreCreateOptions.accessTokenExpiresIn ||
        this.defaultUserStoreOptions.accessTokenExpiresIn,
      refreshTokenExpiresIn:
        userStoreCreateOptions.refreshTokenExpiresIn ||
        this.defaultUserStoreOptions.refreshTokenExpiresIn,
      defaultHashOptions:
        userStoreCreateOptions.defaultHashOptions ||
        this.defaultUserStoreOptions.defaultHashOptions,
    };
  ...
  public passwordHash_create(
    options: PasswordHashCreateOptions
  ): Promise<UserPassword> {
    return new Promise<UserPassword>((resolve, reject) => {
      let hashOptions: HashOptions =
        options.hashOptions || this.userStoreOptions.defaultHashOptions;
        ...
        (err, derivedKey) => {
          ...
          let userPassword: UserPassword = {
            ...
            hashOptions: {
              ...
              keyLength: hashOptions.keyLength,
              digest: hashOptions.digest,
            },
          };
  ...
  public usernamePassword_validate(
    options: UsernamePasswordValidateOptions
  ): Promise<User> {
    return new Promise<User>((resolve, reject) => {
      try {
        let userGetByUsernameOptions: UserGetByUsernameOptions = {
          username: options.username,
        };
        this.user_getByUsername(userGetByUsernameOptions)
          .then((user) => {
            if (user === null) {
              reject(
                new ErrorMessage({
                  ...
                  code: 404,
                  debugData: { username: options.username },
                })
              );
            } else {
              let passwordValidateOptions: PasswordValidateOptions = {
                password: options.password,
                salt: user!.password!.salt,
                hash: user!.password!.hash,
                hashOptions: user!.password!.hashOptions,
              };
              this.password_validate(passwordValidateOptions)
                .then((valid) => {
                  if (valid) {
                    resolve(user);
                  } else {
    ...
    return new Promise<RefreshToken>((resolve, reject) => {
      try {
        ...
        let expires_in: number =
          options.expires_in || this.userStoreOptions.refreshTokenExpiresIn;
  ...
  public accessToken_create(
    ...
    return new Promise<AccessToken>((resolve, reject) => {
      try {
        let now: Date = new Date();
        let expires_in: number =
          options.expires_in || this.userStoreOptions.accessTokenExpiresIn;
        let expires_on: Date = new Date();
        expires_on.setSeconds(now.getSeconds() + expires_in);
  ...
  public accessToken_get(
    options: AccessTokenGetOptions
  ...
  public user_getByUsername(
    options: UserGetByUsernameOptions
  ): Promise<User | null> {
    throw new Error("Method not implemented.");
  }
