...
import type { AccessToken, AccessTokenCreateOptions, LoginData, RefreshToken, User, UsernamePasswordValidateOptions, iUserStore } from "./UserStore";
...
export type LoginOptions = {
   userStore: iUserStore
    ...
    public validateAccessTokenHandler(req: Request, res: Response, next: NextFunction) {
        try{
            if(req.headers && req.headers.authorization && req.headers.authorization.indexOf("Bearer ") === 0){
                ...
                this.options.userStore.accessToken_get({access_token: access_token}).then((accessToken) => {
                    if(accessToken !== null){
    ...
    public loginHandler(req: Request, res: Response, next: NextFunction) {
        ...
        if(req.body && req.body.grant_type === "refresh_token"){
            try {
                ...
                this.options.userStore.refreshToken_get({refresh_token: req.body.refresh_token})
                .then((refreshToken) => {
                    if(refreshToken === null){
                        let errMsg = new ErrorMessage({error:"login refresh_token not found " + req.body.refresh_token, msg:"Invalid RefreshToken", statusCode:401, request:req, debugData:{refresh_token:req.body.refresh_token}});
                        debug('warning', 'refreshToken_upsert',  errMsg);
                        debug("debug", "login refresh_token not found", req.body.refresh_token);
                        res.status(401).json({ "msg": "Invalid RefreshToken!", "error": "Invalid RefreshToken"});
                    }else{
                        var loginData : LoginData = refreshToken.loginData 
                        var accessTokenCreate : AccessTokenCreateOptions = {refreshToken: refreshToken};
                        this.options.userStore.accessToken_create({refreshToken: refreshToken})
                        .then((accessToken) => {
                            this.options.userStore.user_clean({user:loginData.user!})
                            .then(
                                (cleanedUser) => {
                            ...
                            .catch((err) => {
                                ErrorMessage.handleHttpRequestError(req, res, err, "loginHandler");
                            });
                        })
                        .catch((err) => {
                            ErrorMessage.handleHttpRequestError(req, res, err, "loginHandler");
            ...
            let usernamePasswordValidate : UsernamePasswordValidateOptions = {username:req.body.username, password:req.body.password};
            this.options.userStore.usernamePassword_validate(req.body).then(
                (user:User) => {
                    try {
                        this.options.userStore.refreshToken_create({loginData:loginData, refresh_token:"", token_type:"password"})
     ...
     public logoutHandler =  function (req: Request, res: Response, next: NextFunction) {
     
        if (!res.locals.accessToken) {
            if(req.headers && req.headers.authorization && req.headers.authorization.indexOf("Bearer ") === 0){
                res.locals.accessToken = {refresh_token:req.query.refresh_token}
            }else{
                res.status(401).json({ "msg": "User Not Logged In!", "error": "Authorization Header Missing"});
                return;
            }
        }
        ...
        logoutPromises.push(this.options.userStore.refreshToken_delete({refresh_token: refresh_token}));
        logoutPromises.push(this.options.userStore.accessToken_delete({refresh_token: refresh_token}))
