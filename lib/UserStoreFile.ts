...
import type { RefreshTokenCreateOptions, AccessTokenCreateOptions, AccessTokenDeleteOptions, RefreshTokenDeleteOptions, AccessTokenGetOptions, RefreshTokenGetOptions, RefreshToken, AccessToken, iUserStore, User, UserGetOptions, UserGetByEmailOptions, UserGetByUsernameOptions, UserDeleteOptions,  PasswordHashCreateOptions, UserStoreCreateOptions, UserCreateSuperUser, UserCreateSuperUserOptions } from "./UserStore";
...
const debug = debugModule("DeHelper:UserStoreFile"); 
    ...
    export class UserStoreFile extends  UserStore implements iUserStore
    {
        ...
        constructor(userStoreFileCreateOptions : UserStoreFileCreateOptions | undefined){
            ...
            userStoreFileOptions = {
                dataPath : userStoreFileCreateOptions.dataPath || this.defaultUserStoreFileOptions.dataPath,
                memoryOnly : userStoreFileCreateOptions.memoryOnly || this.defaultUserStoreFileOptions.memoryOnly,
            }
        ...
        public user_create_superuser(options: UserCreateSuperUserOptions): Promise<UserCreateSuperUser> {
            ...
            let createHashOptions : PasswordHashCreateOptions = {password: password, salt: "", hashOptions: this.defaultUserStoreOptions.defaultHashOptions}
            this.passwordHash_create(createHashOptions).then((result)=>{
                if(superUser.password === null){
                    superUser.password = {hash:"", salt:"", hashOptions: this.defaultUserStoreOptions.defaultHashOptions };
                }
                superUser.password.hash = result.hash;
                superUser.password.salt = result.salt;
                superUser.password.hashOptions = result.hashOptions;

                this.user_upsert(superUser).then((user)=>{
        ...
        */
        private setup() {
            const nosqlDataDirectory = path.join(this.userStoreFileOptions.dataPath,"nosql");
            if(fs.existsSync(nosqlDataDirectory) === false){
                fs.mkdirSync(nosqlDataDirectory,{recursive:true})
            };
            let refreshTokensFilePath = path.join(nosqlDataDirectory, this.dataFileNames.refreshTokensFile);
            let accessTokensFilePath = path.join(nosqlDataDirectory, this.dataFileNames.accessTokensFile);
            let loginTokensFilePath = path.join(nosqlDataDirectory, this.dataFileNames.loginTokensFile);
            let usersFilePath = path.join(nosqlDataDirectory, this.dataFileNames.usersFile);
            let rolesFilePath = path.join(nosqlDataDirectory, this.dataFileNames.rolesFile);
        ...
        public refreshToken_upsert (refreshToken: RefreshToken) : Promise<RefreshToken > {
            return new Promise<RefreshToken >((resolve, reject) => {
                try {
                    ...
                    this.databases.refreshTokens.update(refreshToken, true).make(function (filter : any) {
                        filter.where('refresh_token', refreshToken.refresh_token);
                        filter.callback(function (err: any, count: number, newRefreshToken: RefreshToken) {
        ...
        public refreshToken_get (options: RefreshTokenGetOptions) : Promise<RefreshToken > {
            return new Promise<RefreshToken >((resolve, reject) => {
                try {
                    ...
                    this.databases.refreshTokens.first().make(function (filter: any) {
                        ...
                        filter.callback(function (err: any, count: number, RefreshToken: RefreshToken) {
                        ...
                        filter.callback(function (err: any, count: number) {
                            ...
                            if (err) {
                                ...
                                this.databases.accessTokens.remove().make(function (filter: any) {
                                    filter.where('refresh_token', options.refresh_token);
                                    filter.callback(function (err: any, count: number) {
                                        debug("debug", "refreshToken_delete", "accessTokens.remove", count);
                                        if (err) {
                                            let errMsg = new ErrorMessage(err);
                                            debug('error', 'refreshToken_delete', "accessTokens.remove",  errMsg);
                                            reject(errMsg);
                                        }else{
                                            resolve();
                                        }
        ...
        public accessToken_upsert (accessToken: AccessToken) : Promise<AccessToken > {
            return new Promise<AccessToken >((resolve, reject) => {
                try {
                    ...
                    this.databases.accessTokens.update(accessToken, true).make(function (filter: any) {
                        filter.where('access_token', accessToken.access_token);
                        filter.callback(function (err: any, count: number, newAccessToken: AccessToken) {
            ...
            return new Promise<AccessToken >((resolve, reject) => {
                try {
                    ...
                    this.databases.accessTokens.first().make(function (filter: any) {
                        ...
                        filter.callback(function (err: any, count: number, accessToken: AccessToken) {
            ...
            return new Promise<Array<User> >((resolve, reject) => {
                try {
                    this.databases.users.find().make(function (filter: any) {
                        filter.callback(function (err: any, count: number, users: Array<User>) {
        ...
        public refreshToken_create (options: RefreshTokenCreateOptions) : Promise<RefreshToken> {
            return new Promise<RefreshToken>((resolve, reject) => {
                try {
                    super.refreshToken_create(options).then((refreshToken) => {
        ...
        public accessToken_create (options: AccessTokenCreateOptions) : Promise<AccessToken> {
            return new Promise<AccessToken>((resolve, reject) => {
                try {
                    super.accessToken_create(options).then((accessToken) => {
